...
/* 13) SP: Dashboard metrics para frontend */
...
BEGIN
    ...
    -- 2. Obtener promedio por vendedor
    SELECT
    ...
    -- 4. Obtener desempeño por vendedor y semana
    SELECT
        v.Nombre AS Vendedor,
        MAX(CASE WHEN vw.WeekNo = 1 THEN vw.TotalUSD END) AS S1,
        MAX(CASE WHEN vw.WeekNo = 2 THEN vw.TotalUSD END) AS S2,
        MAX(CASE WHEN vw.WeekNo = 3 THEN vw.TotalUSD END) AS S3,
        MAX(CASE WHEN vw.WeekNo = 4 THEN vw.TotalUSD END) AS S4,
        MAX(CASE WHEN vw.WeekNo = 5 THEN vw.TotalUSD END) AS S5,
        SUM(vw.TotalUSD) AS Total
    FROM ventas.Vendedores v
    LEFT JOIN ventas.VentasSemanales vw 
        ON vw.SellerId = v.SellerId 
        AND vw.YearMonth = @YearMonth
    WHERE v.Activo = 1
    GROUP BY v.SellerId, v.Nombre
    ORDER BY SUM(ISNULL(vw.TotalUSD, 0)) DESC;
END
GO



/* 14) Diagnóstico de datos */

-- 1. Verificar el mes actual:
SELECT LEFT(CONVERT(VARCHAR(10), GETDATE(), 23), 7) AS MesActual;

-- 2. Ver qué meses tienen datos:
SELECT DISTINCT YearMonth, COUNT(*) as NumeroRegistros
FROM ventas.VentasSemanales
GROUP BY YearMonth
ORDER BY YearMonth;

-- 3. Ver datos del mes actual por vendedor y semana:
DECLARE @ym CHAR(7)=LEFT(CONVERT(VARCHAR(10),GETDATE(),23),7);
SELECT 
    v.Nombre AS Vendedor,
    vs.WeekNo AS Semana,
    vs.TotalUSD
FROM ventas.Vendedores v
LEFT JOIN ventas.VentasSemanales vs ON vs.SellerId = v.SellerId AND vs.YearMonth = @ym
WHERE v.Activo = 1
ORDER BY v.Nombre, vs.WeekNo;

-- 4. Regenerar datos del mes actual:
DECLARE @BaseMin DECIMAL(12,2) = 8000;
DECLARE @BaseMax DECIMAL(12,2) = 15000;
...
SELECT 
    ...
    CAST(@BaseMin + (ABS(CHECKSUM(NEWID())) % (1 + (@BaseMax - @BaseMin))) AS DECIMAL(12,2))
FROM ventas.Vendedores v
CROSS APPLY ventas.fn_WeekRangesForMonth(@Y, @M) w
WHERE v.Activo = 1
    AND NOT EXISTS (
        SELECT 1 FROM ventas.VentasSemanales t
        WHERE t.SellerId = v.SellerId 
        AND t.YearMonth = @ym 
        AND t.WeekNo = w.WeekNo
    );
GO

-- 5. Verificar totales actualizados:
DECLARE @ym CHAR(7)=LEFT(CONVERT(VARCHAR(10),GETDATE(),23),7);

-- Total mensual:
SELECT SUM(v.TotalUSD) AS TotalMensualUSD
FROM ventas.VentasSemanales v 
WHERE v.YearMonth = @ym;
